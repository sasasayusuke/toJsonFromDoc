...
def main(app):
    ...
    try:
        ...
        for file_name in os.listdir(config.DOCUMENT_DIRECTORY):
            match = re.match(r'([^_]+)_(.*)_画面設計書\.xlsx', file_name)
            if match:
                # パターンにマッチした場合
        ...
        for interface in interfaces:
            ...
            if app.radio_value.get() == "JSON":
                ...
                if isinstance(interface["unique_id"], int):
                    api_url = f"{server}/api/items/{interface["unique_id"]}/createsite"
                else:
            ...
            if app.radio_value.get() == "JSON":
                ...
                app.log_output(f"{server}/items/{response.json()["Id"]}/index を作成しました")
