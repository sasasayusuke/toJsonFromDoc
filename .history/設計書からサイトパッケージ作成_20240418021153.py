...
def main(app):
    ...
    try:
        ...
        for interface in interfaces:
            ...
            ### 詳細_編集要素 の 取り込み ###
            sht = workbook[config.SHEET_NAME_EDITOR]
            app.log_output(f"{config.SHEET_NAME_EDITOR} の 取り込みを開始")
            # 型情報データを取得する
            ...
            ### 一覧_画面レイアウト の 取り込み ###
            sht = workbook[config.SHEET_NAME_LAYOUT]
            app.log_output(f"{config.SHEET_NAME_LAYOUT} の 取り込みを開始")
            # 型情報データを取得する
                ...
                for i in range(start_row, end_row):
                    ...
                    for j in range(start_col + 1, end_col):
                        ...
                        if target_cell.value not in label_text_list:
                            app.log_output(f"{util.get_address(target_cell.row, target_cell.column)}: {config.SHEET_NAME_EDITOR} に登録されていないのでスキップします。: {target_cell.value} ", "warning")
                            continue
                        if target_cell.value in list:
                            app.log_output(f"{util.get_address(target_cell.row, target_cell.column)}: ２重に登録されているのでスキップします。: {target_cell.value} ", "warning")
                            continue
                        list.append(target_cell.value)
                if cur_cell.value in config.TABS:
                    if cur_cell.value == "一覧要素":
                        site_json["SiteSettings"]["GridColumns"] = list
                        print(list)

                    # site_json["SiteSettings"]["GridColumns"].append(list)
                    print(list)
